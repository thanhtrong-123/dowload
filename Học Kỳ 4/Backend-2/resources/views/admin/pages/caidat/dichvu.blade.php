                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="sparkline12-list" style="padding-bottom: 0px;">
                            ...
                            <div class="sparkline12-graph">
                                <form id="form-cai-dat-dich-vu" method="POST" enctype="application/x-www-form-urlencoded">
                                    @csrf
                                    <div class="row">
                                        <div class="form-group col-md-3">
                                            <label>Số lượng danh mục dịch vụ</label>
                                            <input type="number" readonly onKeyPress="return khongNhapKyTu(event);" class="form-control" placeholder="Nhập số lượng danh mục dịch vụ" required="" name="so_luong_danh_muc" oninvalid="this.setCustomValidity('Vui lòng điền trường này')" oninput="setCustomValidity('')" value="{{ $cai_dat_dich_vu->so_luong_danh_muc }}">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label>Số lượng thể loại dịch vụ</label>
                                            <input type="number" readonly onKeyPress="return khongNhapKyTu(event);" class="form-control" placeholder="Nhập số lượng thể loại dịch vụ" required="" name="so_luong_the_loai" oninvalid="this.setCustomValidity('Vui lòng điền trường này')" oninput="setCustomValidity('')" value="{{ $cai_dat_dich_vu->so_luong_the_loai }}">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label>Số lượng tất cả dịch vụ</label>
                                            <input type="number" readonly onKeyPress="return khongNhapKyTu(event);" class="form-control" placeholder="Nhập số lượng tất cả dịch vụ" required="" name="so_luong_tat_ca" oninvalid="this.setCustomValidity('Vui lòng điền trường này')" oninput="setCustomValidity('')" value="{{ $cai_dat_dich_vu->so_luong_tat_ca }}">
                                        </div>
                                        <div class="form-group col-md-3">
                                            <label>Số lượng tìm kiếm dịch vụ</label>
                                            <input type="number" readonly onKeyPress="return khongNhapKyTu(event);" class="form-control" placeholder="Nhập số lượng tìm kiếm dịch vụ" required="" name="so_luong_tim_kiem" oninvalid="this.setCustomValidity('Vui lòng điền trường này')" oninput="setCustomValidity('')" value="{{ $cai_dat_dich_vu->so_luong_tim_kiem }}">
                                        </div>
                                    </div>
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-6">
                                            <button type="button" class="btn btn-sm btn-danger" name="destroy" style="display: none;" onclick="destroyDichVu()">Hủy bỏ</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="sparkline12-list">
                            ...
                            <div class="sparkline12-graph">
                                ...
                                <?php 
                                    $arrTuKhoa = explode(',', $cai_dat_dich_vu->tu_khoa);
                                ?>
                                <div class="row">
...
@section('admin_script')
   <script>
        ...
        function edit(){
            let listInputs = $('#form-cai-dat-dich-vu input')
        ...
        $('#form-cai-dat-dich-vu').submit((e) => {
            $.ajax({
                type: 'POST',
                url: 'quantri/caidat/dichvu/chinhsua',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data: $('#form-cai-dat-dich-vu').serialize(),
                success: (response) =>{
        ...
        function deleteTuKhoa(tukhoa){
            $.ajax({
                type: 'POST',
                url: 'quantri/caidat/dichvu/xoatukhoa',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                data: {
        ...
        $('#form-cai-dat-tu-khoa').submit((e) => {
            $.ajax({
                type: 'POST',
                url: 'quantri/caidat/dichvu/themtukhoa',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
