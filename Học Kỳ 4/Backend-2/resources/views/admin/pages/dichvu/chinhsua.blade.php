                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="sparkline12-list">
                            ...
                            <div class="sparkline12-graph">
                                <form id="form-them-tin-tuc" action="quantri/dichvu/chinhsua/{{$dich_vu->id}}" method="POST" enctype="multipart/form-data">
                                	@csrf
                                    <div class="row">
                                        <div class="form-group col-md-6">
                                           ...
                                           <div class="form-select-list">
                                                <select class="form-control custom-select-value" name="id_danh_muc_dich_vu" required="" onchange="changeDanhMuc(this.value)" oninvalid="this.setCustomValidity('Vui lòng chọn danh mục')" oninput="setCustomValidity('')">
                                                    ...
                                                    @if($danh_muc->id == $dich_vu->danh_muc_id)
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <div class="form-select-list">
                                                <select class="form-control custom-select-value" name="id_loai_dich_vu" required="" oninvalid="this.setCustomValidity('Vui lòng chọn loại dịch vụ')" oninput="setCustomValidity('')">
                                                    ...
                                                    @foreach($loai_dich_vu as $loai)
                                                    @if($loai->id == $dich_vu->loai_id)
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-12">
                                            <label>Tiêu đề</label>
                                            <input type="text" class="form-control" placeholder="Nhập tiêu đề" required="" name="tieu_de" oninvalid="this.setCustomValidity('Vui lòng điền trường này')" oninput="setCustomValidity('')" value="{{$dich_vu->tieu_de}}">
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="form-group col-md-12">
                                            <label>Mô tả</label>
                                            <textarea class="form-control" placeholder="Mô tả ngắn" rows="4" name="mo_ta" required="">{{$dich_vu->mo_ta}}</textarea>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="form-group col-md-12">
                                            ...
                                            <div id="preview">
                                                <img src="uploads/images/services/{{ $dich_vu->hinh_anh}}" alt="Hình ảnh" style="margin-left: auto; margin-right: auto; margin-top: 20px; max-width: 400px; display: block;" />
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <div class="responsive-mg-b-30" style="margin-bottom: 30px;">
                                                <textarea id="summernote1" name="noi_dung">{!! $dich_vu->noi_dung !!}</textarea>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
...
@section('admin_script')
   ...
   <script>
        ...
        function changeDanhMuc(str){
            if (str == "") {
                $('#form-them-tin-tuc select[name*=id_loai_dich_vu]').html('<option value="">Chọn loại</option>')
                return
            }
            $.ajax({
                type: 'POST',
                ...
                success: function(response){
                    if(response.status){
                        ...
                        $('#form-them-tin-tuc select[name*=id_loai_dich_vu]').html(str)
                    } else {
                        $('#form-them-tin-tuc select[name*=id_loai_dich_vu]').html('<option value="">Chọn loại</option>')
