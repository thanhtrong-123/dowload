		...
		str += '<div class="modal-content">'
			...
			str += '<div class="modal-body modal-add">'
				str += '<form id="form-add" method="POST" enctype="application/x-www-form-urlencoded">'
					str += '<div class="row">'
						str += '<div class="form-group col-md-12">'
							...
							str += '<div class="form-select-list">'
								str += '<select class="form-control custom-select-value" name="id_danh_muc_sp" onfocus="focusid_danh_muc(this)">'
								str += '<option value="">Chọn danh mục</option>'
								for (var i = 0; i < danhmucsanpham.length; i++) {
									str += '<option value="' + danhmucsanpham[i].id + '">' + danhmucsanpham[i].ten + '</option>'
								}
								str += '</select>'
					...
					str += '<div class="row">'
						str += '<div class="form-group col-md-12">'
							str += '<label>Tên loại sản phẩm</label>'
							str += '<input type="text" class="form-control" placeholder="Nhập tên loại sản phẩm" name="ten" onfocus="focusten(this)">'
...
function saveAdd(){
	if($('#form-add select[name*=id_danh_muc_sp]').val().trim().length == 0) {
	...
	$.ajax({
        ...
        success: (response) => {
            ...
            if(response.status){
            	let strOption = ''
            	strOption += '<button title="Chỉnh sửa" class="pd-setting-ed" onclick="editID('+ response.loai_san_pham.id + ', this)">'
            		strOption += '<i class="fa fa-pencil-square-o" aria-hidden="true"></i>'
            	strOption += '</button>'
            	strOption += '<button title="Xóa" class="pd-setting-ed" onclick="deleteID('+ response.loai_san_pham.id + ');">'
            	...
            	row.push({
            		'state': '',
            		'id': response.loai_san_pham.id,
            		'ten_danh_muc': response.loai_san_pham.ten_danh_muc,
            		'ten': response.loai_san_pham.ten,
            		'option': strOption,
            	})
...
function editID(id, thisbutton){
    $.ajax({
    	type: 'GET',
    	url: 'quantri/loaisanpham/chinhsua/' + id,
    	success: (response) => {
    		if(response.status){
    			let str = ''
				str += '<div class="modal-dialog">'
					str += '<div class="modal-content">'
						str += '<div class="modal-header header-color-modal bg-color-3" style="background: #2b982b; color: white;">'
							str += '<h4 class="modal-title">Chỉnh sửa loại sản phẩm</h4>'
							str += '<div class="modal-close-area modal-close-df">'
								str += '<a class="close" data-dismiss="modal" href="javascript:void(0)"><i class="fa fa-close"></i></a>'
							str += '</div>'
						str += '</div>'
						str += '<div class="modal-body modal-add">'
							str += '<form id="form-edit" method="POST" enctype="application/x-www-form-urlencoded">'
								str += '<div class="row">'
									str += '<div class="form-group col-md-12">'
										...
										str += '<div class="form-select-list">'
											str += '<select class="form-control custom-select-value" name="id_danh_muc_sp" onfocus="focusid_danh_muc(this)">'
											str += '<option value="">Chọn danh mục</option>'
											for (var i = 0; i < response.danh_muc_san_pham.length; i++) {
												if(response.danh_muc_san_pham[i].id == response.loai_san_pham.id_danh_muc_sp){
													str += '<option value="' + response.danh_muc_san_pham[i].id + '" selected>' + response.danh_muc_san_pham[i].ten + '</option>'
												} else {
													str += '<option value="' + response.danh_muc_san_pham[i].id + '">' + response.danh_muc_san_pham[i].ten + '</option>'
												}
											}
											str += '</select>'
											str += '<p class="text-danger" id="validateRequiredSelect" style="display:none">Vui lòng chọn trường này</p>'
								...
								str += '<div class="row">'
									str += '<div class="form-group col-md-12">'
										str += '<label>Tên loại sản phẩm</label>'
										str += '<input type="text" class="form-control" placeholder="Nhập tên loại sản phẩm" name="ten" onfocus="focusten(this)" value="' + response.loai_san_pham.ten + '">'
										str += '<p class="text-danger" id="validateEmpty" style="display:none">Vui lòng điền vào trường này</p>'
										str += '<p class="text-danger" id="validateMaxLength" style="display:none">Tên không được vượt quá 100 ký tự</p>'
									str += '</div>'
								str += '</div>'
								str += '<div class="row">'
									str += '<div class="form-group col-md-12 text-center">'
										str += '<button class="btn btn-success" type="button" onclick="saveEdit(' + response.loai_san_pham.id + ', ' + $(thisbutton).parent().parent().data('index') + ')">Lưu lại</button>'
									str += '</div>'
								str += '</div>'
							str += '</form>'
						str += '</div>'
...
function saveEdit(id, index){
	if($('#form-edit select[name*=id_danh_muc_sp]').val().trim().length == 0) {
		$('#validateRequiredSelect').css('display', 'inline')
		return
	}
...
function deleteID(id) {
    ...
    .then((isConfirm) => {
        if (isConfirm) {
            $.ajax({
                type: 'GET',
                url: 'quantri/loaisanpham/xoa/' + id,
                success: (response) => {
                    if(response.status){
